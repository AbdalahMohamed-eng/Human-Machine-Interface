...
5 Decimal Temperature Values Accepted
...
6 Manual Button High Limit Increase
    Log To Console    Pausing for 5 seconds. Please press the HIGH INC button manually.
    Sleep    5s
    ${resp}=    Read Response
    Should Contain    ${resp}    OK: High limit set to

7 Manual Button High Limit Decrease
    Log To Console    Pausing for 5 seconds. Please press the HIGH DEC button manually.
    Sleep    5s
    ${resp}=    Read Response
    Should Contain    ${resp}    OK: High limit set to

8 Manual Button Low Limit Increase
    Log To Console    Pausing for 5 seconds. Please press the LOW INC button manually.
    Sleep    5s
    ${resp}=    Read Response
    Should Contain    ${resp}    OK: Low limit set to

9 Manual Button Low Limit Decrease
    Log To Console    Pausing for 5 seconds. Please press the LOW DEC button manually.
    Sleep    5s
    ${resp}=    Read Response
    Should Contain    ${resp}    OK: Low limit set to

10 LEDs Off in Safe Range
...
11 Blue LED ON Below Low Limit
    Send Command    set_temp=17
    ${resp}=    Read Response
    Verify Status     ON     OFF     OFF

12 Blue LED Turns On At Exact Low Limit
...
13 Blue LED Off above Low Limit
    Send Command    set_temp=21
    ${resp}=    Read Response
    Verify Status     OFF     OFF     OFF

14 Red LED, Fan and Buzzer ON Above High Limit
    Send Command    set_temp=36
    ${resp}=    Read Response
    Verify Status     OFF     ON     ON

15 Red LED, Fan and Buzzer Turn On At Exact High Limit
    Send Command    set_low=20
    ${resp}=    Read Response
    WHILE    "OK" not in $resp
        ${resp}=    Read Response
...
16 System Returns to Safe State
    Send Command    set_temp=28
    ${resp}=    Read Response
    Verify Status     OFF     OFF     OFF

17 OK/NOK Display on LCD
